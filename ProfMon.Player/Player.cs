...
namespace ProfMon.Player {
    public class Player : NamedProfObj {
        ...
        public void ModifyItemCount (ID itemCatagory, Item item, int count) {
            var itemStack = _inventory[ID].First(i => i.Item.ID == item.ID);
            _inventory[ID].First(i => i.Item.ID == item.ID).ModifyStack(count);
